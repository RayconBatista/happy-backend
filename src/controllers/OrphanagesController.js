...
exports.default = {
    async index(request, response) {
        ...
        return response.json({
            result: orphanages_view_1.default.renderMany(orphanages)
        });
    },
    async show(request, response) {
        ...
        return response.json({ result: orphanages_view_1.default.render(orphanage) });
    },
    async create(request, response) {
        const { name, latitude, longitude, about, instructions, open_hours, open_on_weekends, } = request.body;
        const orphanagesRepository = typeorm_1.getRepository(Orphanage_1.default);
        const requestImg = request.files;
